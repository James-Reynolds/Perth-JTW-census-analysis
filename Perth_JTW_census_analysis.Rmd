...
# 2.1 Journey To Work (JTW) by Statistical Area 1 (SA1) of usual residence
JTW data was extracted for people whose place of usual residence is in Greater Perth using the Table Builder on the ABS website. The 15 travel modes data was extracted^[This groups people into those who travelled by train, bus, ferry, tram, taxi, car (as driver), car (as passenger), truck, motorbike, bicycle, walked only, other mode, worked at home, did not go to work, not stated and not applicable. Note that there are slight differences in the variable names between the 2016 and 2021 censuses: taxi becomes taxi/ride share, tram becomes LRT/tram. Here the 2016 names will be adopted.].
...
SA1_UR_by_MTW15P_2016_Greater_Perth <- read_csv("01 data/Greater_Perth_SA1_UR_by_MTW15P_2016.csv", 
    skip = 8)

SA1_UR_by_MTW15P_2021_Greater_Perth <- read_csv("01 data/Greater_Perth_SA1_UR_by_MTW15P_2021.csv", 
    skip = 8)

#Drop blank top row, total row and bottom rows with copyright information
SA1_UR_by_MTW15P_2016_Greater_Perth <- SA1_UR_by_MTW15P_2016_Greater_Perth[2:(nrow(SA1_UR_by_MTW15P_2016_Greater_Perth)-5),]
SA1_UR_by_MTW15P_2021_Greater_Perth <- SA1_UR_by_MTW15P_2021_Greater_Perth[2:(nrow(SA1_UR_by_MTW15P_2021_Greater_Perth)-5),]

#Update first column name to be SA1_CODE_2016, and rest of columns to match 2016 column names
names(SA1_UR_by_MTW15P_2016_Greater_Perth) <- 
  c("SA1_CODE_2016", 
    names(SA1_UR_by_MTW15P_2016_Greater_Perth[,2:ncol(SA1_UR_by_MTW15P_2016_Greater_Perth)]))
#Update first column name to be SA1_CODE_2021
names(SA1_UR_by_MTW15P_2021_Greater_Perth) <- 
  c("SA1_CODE_2021", 
    names(SA1_UR_by_MTW15P_2021_Greater_Perth[,2:ncol(SA1_UR_by_MTW15P_2016_Greater_Perth)]))

#clean the names
SA1_UR_by_MTW15P_2016_Greater_Perth <- clean_names(SA1_UR_by_MTW15P_2016_Greater_Perth)
SA1_UR_by_MTW15P_2021_Greater_Perth <- clean_names(SA1_UR_by_MTW15P_2021_Greater_Perth)

#drop total column
SA1_UR_by_MTW15P_2016_Greater_Perth <- tibble(SA1_UR_by_MTW15P_2016_Greater_Perth[,1:17])
SA1_UR_by_MTW15P_2021_Greater_Perth <- tibble(SA1_UR_by_MTW15P_2021_Greater_Perth[,1:17])



```

The ABS provides a correspondence file to convert between 2016 and 2021 SA1 zones. This file ^[CG_SA1_2016_SA1_2021.csv] is loaded and used to convert the SA1 2016 data to SA1 2021. 

```{r SA1_UR_correspndence files, echo=FALSE, message = FALSE, warning=FALSE, fig.margin=TRUE, fig.cap="Greater Perth, car driver mode share by destination zone, 2016", cache=TRUE}
...
CG_SA1_2016_SA1_2021 <- clean_names(CG_SA1_2016_SA1_2021)
#For some reason the correspondence file provides SA1_MAINCODE_2016, rather than SA1_CODE_2016. The main code is a different number, but there is a correspondance in the SA1_2016_AUST file from https://www.abs.gov.au/AUSSTATS/abs@.nsf/DetailsPage/1270.0.55.001July%202016?OpenDocument. So import that
SA1_2016_AUST <- read_csv("01 data/SA1_2016_AUST.csv")
SA1_2016_AUST <- clean_names(SA1_2016_AUST)
...
SA1_UR_by_MTW15P_2016_Greater_Perth$sa1_7digitcode_2016 <- as.character(SA1_UR_by_MTW15P_2016_Greater_Perth$sa1_7digitcode_2016)
...
SA1_UR_by_MTW15P_2016_Greater_Perth <- left_join(SA1_UR_by_MTW15P_2016_Greater_Perth, CG_SA1_2016_SA1_2021, by = "sa1_7digitcode_2016")

#multiply mode numbers by RATIO_FROM_TO value
SA1_UR_by_MTW15P_2016_Greater_Perth[,36:51] <- SA1_UR_by_MTW15P_2016_Greater_Perth[,2:17] * as.numeric(SA1_UR_by_MTW15P_2016_Greater_Perth$ratio_from_to)
#Drop unneeded columns
SA1_UR_by_MTW15P_2016_Greater_Perth <- tibble(sa1_code_2021 = SA1_UR_by_MTW15P_2016_Greater_Perth$sa1_code_2021, SA1_UR_by_MTW15P_2016_Greater_Perth[,36:51])

```

Next, add the Heritage, North-south and Outer east groupings to the datasets. To do this, however, it is necessary to add the Local Government Areas (LGA) details to each dataset.  LGAs do not directly correspond to SA1 boundaries, so it will be necessary to 1. load the SA1 and DZ geometry to the journey to work datasets, 2. Load the LGA geometry, 3. Find the intersections of the LGA boundaries and the SA1s.  

First, loading the ABS mapping data via the absmapsdata package [@absmapsdata] gives:

```{r load_absmapsdata_SA1_to_LGA, echo=FALSE, fig.margin=TRUE, fig.cap="", cache=TRUE}

#Load SA1_2021 data
sa1_2021_map_data <- sa12021

#Only keep Greater Perth data
sa1_2021_map_data <- sa1_2021_map_data %>% filter(gcc_name_2021 == "Greater Perth")

#join SA1 geometry to SA1 JTW data
SA1_UR_by_MTW15P_2016_Greater_Perth <- left_join(SA1_UR_by_MTW15P_2016_Greater_Perth, sa1_2021_map_data, by = "sa1_code_2021")
SA1_UR_by_MTW15P_2021_Greater_Perth <- left_join(SA1_UR_by_MTW15P_2021_Greater_Perth, sa1_2021_map_data, by = "sa1_code_2021")
```
...
SA1_UR_by_MTW15P_2016_Greater_Perth <- st_sf(SA1_UR_by_MTW15P_2016_Greater_Perth)
SA1_UR_by_MTW15P_2021_Greater_Perth <- st_sf(SA1_UR_by_MTW15P_2021_Greater_Perth)

#find intersections of SA1 and LGA and DZ and LGA boundaries
SA1_UR_by_MTW15P_2016_Greater_Perth <- st_intersection(SA1_UR_by_MTW15P_2016_Greater_Perth, lga_areas)
SA1_UR_by_MTW15P_2021_Greater_Perth <- st_intersection(SA1_UR_by_MTW15P_2016_Greater_Perth, lga_areas)
...
SA1_UR_by_MTW15P_2016_Greater_Perth <- left_join(SA1_UR_by_MTW15P_2016_Greater_Perth, LGA_to_group, by = "lga_name_2021")
SA1_UR_by_MTW15P_2021_Greater_Perth <- left_join(SA1_UR_by_MTW15P_2021_Greater_Perth, LGA_to_group, by = "lga_name_2021")

#Map Groupings
map <- SA1_UR_by_MTW15P_2016_Greater_Perth %>%
    ggplot() +
...
# 2.2 Journey to work by SA1 (usual residence) and destination zone
...
```{r load_absmapsdata_destination_zones_2021_SA1_and_DZ, echo=FALSE, fig.margin=TRUE, fig.cap="Perth 2021 destination zonea", cache=TRUE}
...
```{r load_absmapsdata_SA1_zones_2021_SA1_and_DZ, echo=FALSE, fig.margin=TRUE, fig.cap="Perth 2021 destination zonea", cache=TRUE}
...
names(SA1_UR_by_MTW15P_2021_Greater_Perth) <- 
  c("sa1_code_2021", 
    names(SA1_UR_by_MTW15P_2021_Greater_Perth[,2:ncol(SA1_UR_by_MTW15P_2021_Greater_Perth)]))
...
```{r add_groupings_SA1_and_DZ, echo=FALSE, message=FALSE, warning=FALSE, fig.margin=FALSE, fig.cap="Greater Perth: Heritage, North-south and Outer East groupings", cache=TRUE}
...
names(SA1_UR_by_MTW15P_2016_Greater_Perth) <- c("sa1_code_2021", "Train", "Bus", "Ferry", "Tram", "Taxi", "Car, as driver", "Car, as passenger", "Truck", "Motorbike", "Bicycle", "Walked only", "Other mode", "Worked at home", "Did not go to work", "Not stated", "Not applicable", "sa2_code_2021", "sa2_name_2021", "sa3_code_2021", "sa3_name_2021", "sa4_code_2021", "sa4_name_2021", "gcc_code_2021", "gcc_name_2021", "state_code_2021", "state_name_2021", "areasqkm_2021", "cent_lat", "cent_long", "lga_code_2021", "lga_name_2021", "state_code_2021.1", "state_name_2021.1", "areasqkm_2021.1", "cent_lat.1", "cent_long.1", "geometry", "grouping")
...
```{r add_groupings_DZN_LGA_SA1_and_DZ, echo=FALSE, message=FALSE, warning=FALSE, fig.margin=FALSE, fig.cap="Greater Perth: Heritage, North-south and Outer East groupings", cache=TRUE}
...
# 2.3 Population and working population data
...
SA1_UR_by_MTW15P_2016_Greater_Perth_no_geometry <- SA1_UR_by_MTW15P_2016_Greater_Perth %>% st_drop_geometry()
SA1_UR_by_MTW15P_2021_Greater_Perth_no_geometry <- SA1_UR_by_MTW15P_2021_Greater_Perth %>% st_drop_geometry()

#calculate aggregates for everyone with a usual residence in perth
Aggregate_JTW_2016_2021 <- tibble(SA1_UR_by_MTW15P_2016_Greater_Perth_no_geometry[,1:17] %>% adorn_totals() %>% tail(n=1))
Aggregate_JTW_2016_2021 <- add_row(Aggregate_JTW_2016_2021, SA1_UR_by_MTW15P_2021_Greater_Perth_no_geometry[,1:17] %>% adorn_totals() %>% tail(n=1))
...
SA1_UR_by_MTW15P_2016_Greater_Perth_no_geometry_disaggregate <- split (SA1_UR_by_MTW15P_2016_Greater_Perth_no_geometry,SA1_UR_by_MTW15P_2016_Greater_Perth_no_geometry$grouping)

SA1_UR_by_MTW15P_2021_Greater_Perth_no_geometry_disaggregate <- split (SA1_UR_by_MTW15P_2021_Greater_Perth_no_geometry, SA1_UR_by_MTW15P_2021_Greater_Perth_no_geometry$grouping)

##calculate for everyone with a usual residence in perth, by grouping
#2016
Group_JTW_2016 <- lapply(
  SA1_UR_by_MTW15P_2016_Greater_Perth_no_geometry_disaggregate,  
  function(x) x%>% 
...
Group_JTW_2021 <- lapply(
  SA1_UR_by_MTW15P_2021_Greater_Perth_no_geometry_disaggregate,  
  function(x) x%>% 
    dplyr::select(sa1_code_2021, Train, Bus, Ferry, Tram, Taxi, "Car, as driver", "Car, as passenger", "Truck", "Motorbike", "Bicycle", "Walked only", "Other mode", "Worked at home", "Not stated", "Not applicable")
)
